...
def _resolve_reportable(
    workspace: Workspace,
    relative_path: str,
) -> Path:
    """Resolve a path for read-only reporting (GET_METADATA / EXISTS).

    Unlike resolve_workspace_path, a reparse point in the FINAL position is
    returned unresolved so callers can *report* it (`ItemType.REPARSE_POINT`)
    without ever traversing it. A reparse point in an ancestor position, and
    every other policy rule, remains denied exactly as before.
    """

    try:
        return resolve_workspace_path(workspace, relative_path, allow_root=True)
    except PolicyViolation as violation:
        if violation.code != 'REPARSE_POINT_DENIED':
            raise
        original = violation
    pure = PureWindowsPath(relative_path or '.')
    name = pure.name
    if not name:
        raise original
    parent_raw = str(pure.parent)
    parent = resolve_workspace_path(
        workspace,
        '' if parent_raw == '.' else parent_raw,
        allow_root=True,
    )
    target = parent / name
    if not is_reparse_point(target):
        # The denial came from an ancestor (or the state changed): keep it.
        raise original
    allow_hidden = workspace.policy.get('allow_hidden_items') is True
    if is_hidden(target) and not allow_hidden:
        raise _violation(
            'HIDDEN_ITEM_DENIED',
...
def get_metadata(
    ...
    target = _resolve_reportable(workspace, relative_path)
...
def path_exists(
    ...
    try:
        ...
        # EXISTS is a probe: paths the policy makes invisible are reported as
        # absent instead of surfacing a policy rejection to the caller.
        if violation.code in ('HIDDEN_ITEM_DENIED', 'REPARSE_POINT_DENIED'):
            return False, None
        raise
    if not _path_exists(target):
        return False, None
    ...
    *,
    ...
    for candidate in _iter_search_candidates(
        ...
        if candidate.is_file():
            # SEARCH_FILES returns metadata only (never content), so it
            # mirrors LIST_DIRECTORY visibility instead of the extension
            # allowlist: extensionless and non-allowlisted files are listed.
            # Content access remains guarded by READ_FILE/SEARCH_CONTENT.
            if (
    ...
    # Name searches are fully pageable: each page is bounded by page_limit
    # (already capped by maximum_search_results in _search_limits) and `skip`
    # walks the complete, scan-cap-bounded result set — no result is ever
...
def search_content(
    ...
    *,
    ...
    # always advance past a previously reported cap — the tail is pageable.
    window_end = skip + page_limit
    count_limit = skip + result_cap
    matched = 0
    truncated = False
    for path in candidates:
        ...
        for found in pattern.finditer(text):
            if matched >= count_limit:
                truncated = True
                break
            if skip <= matched < window_end:
                line_number, column_number = _locate_offset(
                    offsets, found.start()
                )
                matches.append(
                    ContentMatch(
